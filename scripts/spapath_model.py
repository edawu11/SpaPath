...
import importlib
import spapath_network
import spapath_utils

importlib.reload(spapath_network)
importlib.reload(spapath_utils)
...
def _print_cluster_size_records(prefix, rows):
    size_text = ", ".join(
        f"{row['cluster']}:{row['count']}({row['fraction']:.3f})" for row in rows
    )
...
class Model:
    def __init__(
        ...
        spapath_utils.seed_everything(self.seed)
        ...
        for i, bid in enumerate(self.section_ids):
            adata_batch = self.adata_full[
                self.adata_full.obs[self.batch_key] == bid
            ].copy()
            if _is_st(self.data_type[i]):
                ...
                adata_batch.obsp["graph_cos"] = self.adata_full.uns[
                    graph_cos_key
                ].copy()
            adata_batch.uns = {}
        ...
        for i in range(self.n_slices):
            ...
            if _is_st(self.data_type[i]):
                ...
                self.adj_matrix_dict[i] = (
                    torch.from_numpy(graph).float().to(self.device)
                )
                ...
                self.graph_cos_dict[i] = (
                    torch.from_numpy(graph_cos).float().to(self.device)
                )
    ...
    def initial_embedding(self):
        ...
        for i in range(self.n_slices):
            ...
            for step in tqdm(
                range(self.n_pre_training_steps),
                desc=f"Pretrain slice {i}",
                disable=not self.verbose,
            ):
                Ss, X_recons = self.netmodel[i](
                    self.node_feats_dict[i], self.adj_matrix_dict[i]
                )
        ...
        for i, bid in enumerate(self.section_ids):
            ...
            self.adata_full.obsm[self.pre_cellembed_key][mask.values, :] = (
                self.S_dict[i].detach().cpu().numpy()
            )
    ...
    def clustering(self, ifmerge=True, init_res=1.5, intopk=30, n_clusters=None):
        ...
        if n_clusters is None:
            ...
            for i in range(self.n_slices):
                ...
                sc.tl.leiden(adata, resolution=init_res, random_state=self.seed)
            ...
            for i in range(self.n_slices):
                if self.pre_cellembed_key not in self.batch_list[i].obsm:
                    raise KeyError(
                        f"{self.pre_cellembed_key} not found in batch_list[{i}].obsm"
                    )
                y_pred_dict[i] = spapath_utils.search_res(
        ...
        if ifmerge:
            for i in range(self.n_slices):
                ...
                if _is_st(self.data_type[i]):
                    ...
                    s_gene_embed = spapath_utils.gene_embed_weight(
                ...
                s_gene2cell_mat = spapath_utils.cell_to_gene_pdistance(
                ...
                while True:
                    current_labels = (
                        self.batch_list[i].obs[self.cluster_key].astype(int).values
                    )
                    ...
                    s_gene_list = spapath_utils.select_sig_genes(
                        self.batch_list[i], label_key=self.cluster_key, topk=intopk
                    )
                    label_order = np.array(
                        [_int_label(label) for label in s_gene_list.keys()]
                    )

                    _, score_mat = spapath_utils.self_compute_gene_pvalue(
                        s_gene_list, self.genename.values
                    )
                    ...
                    if np.any(upper_tri_values <= 0.05):
                        ...
                        self.batch_list[i].obs.loc[
                            mask, self.cluster_key
                        ] = actual_label_i
        ...
        for i in range(self.n_slices):
            ...
            y_combined_last[i] = np.array(
                [label_to_index[label] for label in cluster_labels]
            )
            ...
            mu_dict[i] = nn.Parameter(
                torch.tensor(cluster_centers, dtype=torch.float32, device=self.device)
            )
        ...
        if not self.use_dec_finetune:
            ...
            for i in range(self.n_slices):
                cluster_size_diagnostics[str(i)]["after_dec"] = (
                    cluster_size_diagnostics[str(i)]["after_merge"]
                )
                cluster_size_diagnostics[str(i)]["dec_skipped"] = True
                self.batch_list[i].obsm[self.pre_cellembed_key] = (
                    self.S_dict[i].detach().cpu().numpy()
                )
                self.batch_list[i].obs[self.cluster_key] = y_combined_last[i].astype(
                    str
                )
            ...
            for i, bid in enumerate(self.section_ids):
                ...
                self.adata_full.obsm[self.pre_cellembed_key][mask, :] = self.batch_list[
                    i
                ].obsm[self.pre_cellembed_key]
                self.adata_full.obs.loc[mask, self.cluster_key] = (
                    self.batch_list[i].obs[self.cluster_key].values.astype(str)
                )
        ...
        if self.verbose:
            print(
                "Apply Deep Embedded Clustering (DEC) to fine-tune the clustering results."
            )
        for i in range(self.n_slices):
            for step in range(self.n_training_steps):
                ...
                delta_label = (
                    np.sum(y_pred != y_combined_last[i]).astype(np.float32)
                    / Ss.shape[0]
                )
                y_combined_last[i] = y_pred
                if (
                    step > 0
                    and step % self.update_interval == 0
                    and delta_label < self.tol
                ):
                    break
            ...
            cell_embed.index = [
                f"slice-{i}-cell-{j}" for j in range(cell_embed.shape[0])
            ]
            ...
            if i == 0:
                ...
                self.batch_list[i].obs[
                    self.cluster_key
                ] = cell_pred.values.ravel().astype(str)
    ...
    def integrate(self, topk=30, temp=0.1):
        ...
        if not hasattr(self, "netmodel"):
            raise RuntimeError(
                "Call initial_embedding() and clustering() before integrate()."
            )
        missing_cluster = [
            i
            for i in range(self.n_slices)
        ...
        self.net = spapath_network.IndEmbedNet(
            feature_dim=self.feature_dim,
            latent_dim=self.hidden_dims,
        ...
        for i in range(self.n_slices):
            ...
            self.net.encoder_layers1[i].load_state_dict(
                {
                    k.replace("encoder_layer1.", ""): v
                    for k, v in state_dict.items()
                    if k.startswith("encoder_layer1.")
                }
            )
            self.net.encoder_layers2[i].load_state_dict(
                {
                    k.replace("encoder_layer2.", ""): v
                    for k, v in state_dict.items()
                    if k.startswith("encoder_layer2.")
                }
            )

            self.net.decoder_layers1[i].load_state_dict(
                {
                    k.replace("decoder_layer1.", ""): v
                    for k, v in state_dict.items()
                    if k.startswith("decoder_layer1.")
                }
            )
            self.net.decoder_layers2[i].load_state_dict(
                {
                    k.replace("decoder_layer2.", ""): v
                    for k, v in state_dict.items()
                    if k.startswith("decoder_layer2.")
                }
            )

        self.optimizer_net = torch.optim.Adamax(
        ...
        for step in tqdm(range(self.n_training_steps), disable=not self.verbose):
            ...
            for i in range(self.n_slices):
                if _is_st(self.data_type[i]):
                    s_gene_embed = spapath_utils.gene_embed_weight_torch(
                        self.X_dict[i], Ss[i], self.adj_matrix_dict[i]
                    )
                else:
                    s_gene_embed = spapath_utils.gene_embed_weight_torch(
                        self.X_dict[i], Ss[i]
                ...
                if step % 20 == 0:
                    s_gene2cell_mat = spapath_utils.cell_to_gene_pdistance_torch(
                        Ss[i], s_gene_embed
                    )
                    self.batch_list[i].layers["dist"] = (
                        s_gene2cell_mat.detach().cpu().numpy()
                    )
                    s_gene_list = spapath_utils.select_sig_genes(
                        self.batch_list[i], label_key=self.cluster_key, topk=topk
                    )
            ...
            if step % 20 == 0:
                ...
                for i in range(tar_index):
                    ...
                    _, score_mat = spapath_utils.compute_gene_pvalue(
                        ...
                        self.genename.values,
                    )

                    ref_label_order = [
                        _int_label(label)
                        for label in all_gene_embed_dict[i]["gene_list"].keys()
                    ]
                    tar_label_order = [
                        _int_label(label)
                        for label in all_gene_embed_dict[tar_index]["gene_list"].keys()
                    ]
                    ref_label_to_idx = {
                        label: idx for idx, label in enumerate(ref_label_order)
                    }
                    tar_label_to_idx = {
                        label: idx for idx, label in enumerate(tar_label_order)
                    }
                    ref_clusters_for_anchor = (
                        self.batch_list[i].obs[self.cluster_key].astype(int).values
                    )
                    ...
                    ref_anchor_eligibility[str(i)] = {
                        ...
                        "eligible_clusters": sorted(
                            int(label) for label in eligible_ref_labels
                        ),
                        "filtered_clusters": [
                            int(row["cluster"])
                            for row in ref_anchor_records
                    ...
                    if len(ref_tar_dict[i]) > 0:
                        ...
                        def _pair_still_valid(row):
                            ...
                            if ref_label not in eligible_ref_labels:
                                return False
                            if (
                                ref_label not in ref_label_to_idx
                                or tar_label not in tar_label_to_idx
                            ):
                                return False
                            return (
                                score_mat[
                                    ref_label_to_idx[ref_label],
                                    tar_label_to_idx[tar_label],
                                ]
                                < 0.05
                            )

                        keep_mask = ref_tar_dict[i].apply(_pair_still_valid, axis=1)
                        ref_tar_dict[i] = ref_tar_dict[i][keep_mask].reset_index(
                            drop=True
                        )
                    ...
                    for r_idx, ref_label in enumerate(ref_label_order):
                        ...
                        for t_idx, tar_label in enumerate(tar_label_order):
                            ...
                            candidate_pairs.append(
                                (ref_label, tar_label, score_mat[r_idx, t_idx])
                            )
                    ...
                    if len(eligible_ref_row_idx) > 0:
                        ...
                        far_candidate_idx = np.where(
                            np.min(eligible_score_mat, axis=0) > 0.05
                        )[0]
                        far_candidates_i = set(
                            tar_label_order[t_idx] for t_idx in far_candidate_idx
                        )
                    else:
                        far_candidates_i = set()
                ...
                non_far_cell_indices = np.where(
                    ~np.isin(tar_clusters, global_far_pool)
                )[0]
                ...
                if self.verbose and step % self.step_interval == 0:
                    print(
                        ...
                        f"{sorted(matched_target_clusters)}; far-cluster pool "
                        f"{global_far_pool}."
                    )
            ...
            for i in range(self.n_slices):
                ...
                if _is_st(self.data_type[i]):
                    ...
                    mat_diff = (
                        torch.matmul(Ss_norm_dict[i], Ss_norm_dict[i].T) - graph_cos
                    )
                ...
                for i in range(tar_index):
                    ...
                    if n_pairs == 0:
                        ...
                        for j in range(len(ref_list)):
                            ...
                            mmd_neg_tar = mmd_loss_fun(
                                Ss1_sub, Ss_out_sub, kernel_num=5
                            )
                            mmd_neg_ref = mmd_loss_fun(
                                Ss0_sub, Ss_out_sub, kernel_num=5
                            )
            ...
            loss_total = (
                self.coef_inter * inter_loss
                + self.coef_intra * intra_loss
                + self.coef_recon * recon_loss
                + self.coef_geom * geom_loss
            )
            ...
            if self.verbose and step % self.step_interval == 0:
                print(
                    f"Training step {step}: reconstruction={recon_loss.item():.4f}, "
                    f"inter-cluster={inter_loss.item():.4f}, "
                    f"intra-cluster={intra_loss.item():.4f}, "
                    f"geometry={geom_loss.item():.4f}."
                )
        ...
        for i, bid in enumerate(self.section_ids):
            mask = (self.adata_full.obs[self.batch_key] == bid).values
            self.adata_full.obsm[self.cellembed_key][mask, :] = self.batch_list[i].obsm[
                self.cellembed_key
            ]
        ...
        for i in range(self.n_slices):
            ...
            if _is_st(self.data_type[i]):
                ...
                s_gene_embed = spapath_utils.gene_embed_weight(
                    X_all, adata_batch.obsm[self.cellembed_key], graph
                )
                self.adata_full.uns[f"Slice_{i}_{self.geneembed_key}"] = s_gene_embed
                del X_all, graph
            else:
                s_gene_embed = spapath_utils.gene_embed_weight(
                    X_all, adata_batch.obsm[self.cellembed_key]
                )
                self.adata_full.uns[f"Slice_{i}_{self.geneembed_key}"] = s_gene_embed
                del X_all
            dist_list.append(
                spapath_utils.cell_to_gene_pdistance(
                    adata_batch.obsm[self.cellembed_key], s_gene_embed
                )
            )

        full_dist = np.zeros(
            (self.adata_full.n_obs, self.adata_full.n_vars), dtype=np.float32
        )
        for i, bid in enumerate(self.section_ids):
            mask = (self.adata_full.obs[self.batch_key] == bid).values
        ...
        if self.verbose:
            print(
                "Integration completed; cell embeddings, gene embeddings, and "
                "cell-gene distances were stored."
            )
