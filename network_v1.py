...
class GraphAttentionLayer(nn.Module):
    ...
    """
    def __init__(self, in_features, out_features, dropout, alpha, concat=False,temp=1.0):
...
class SingleNet(nn.Module):
    def __init__(self, feature_dim, latent_dim, data_type):
        ...
        if self.use_gnn:
            self.encoder_layer1 = GraphAttentionLayer(feature_dim, latent_dim[0], dropout=0, alpha=0.2)
            self.encoder_layer2 = DenseLayer(latent_dim[0], latent_dim[1])
            self.decoder_layer1 = GraphAttentionLayer(latent_dim[1], latent_dim[0], dropout=0, alpha=0.2)
    ...
    def decoder(self, S, adj_matrix=None):
        if self.use_gnn:
            H = F.elu(self.decoder_layer1(S, adj_matrix, tied_attention=self.encoder_layer1.attention))
        else:
...
class IndEmbedNet(nn.Module):
    def __init__(self, feature_dim, latent_dim, n_slices, data_type):
        ...
        self.encoder_layers1 = nn.ModuleList([
            self._make_layer1(feature_dim, latent_dim[0], self.use_gnn[i]) for i in range(n_slices)
        ])
        ...
        self.decoder_layers1 = nn.ModuleList([
            self._make_layer1(latent_dim[1], latent_dim[0], self.use_gnn[i]) for i in range(n_slices)
        ])
    ...
    def decoder(self, i, S, adj_matrix=None):
        if self.use_gnn[i]:
            H = F.elu(self.decoder_layers1[i](S, adj_matrix, tied_attention=self.encoder_layers1[i].attention))
        else:
